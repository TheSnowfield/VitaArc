...
int _pthread_create(pthread_t *tid,
                    ...
                    void *arg)
{
  log_v(TAG, "_pthread_create(%d, %d, %d, %d)", tid, attr, start, arg);
  return pthread_create(tid, attr, start, arg);
}

int _pthread_detach(pthread_t tid)
{
  log_v(TAG, "_pthread_detach(%d)", tid);
...
int _pthread_equal(pthread_t t1,
                   pthread_t t2)
{
  log_v(TAG, "_pthread_detach(%d, %d)", t1, t2);
                        ...
                        const pthread_mutexattr_t *attr)
{
  ...
  log_v(TAG, "_pthread_mutex_init(%d, %d)", _MUTEX(mutex), attr);
...
{
  int result = pthread_mutex_destroy(&_MUTEX(mutex));
  {
    // cleanup
    _MUTEX(mutex) = NULL;
    log_v(TAG, "_pthread_mutex_destroy(%d)", _MUTEX(mutex));
  }
  return result;
}
...
{
  log_v(TAG, "_pthread_mutex_lock(%d)", _MUTEX(mutex));
...
{
  log_v(TAG, "_pthread_key_create(%d, %d)", key, destructor);
  return pthread_key_create(key, destructor);
}

int _pthread_key_delete(pthread_key_t key)
{
  log_v(TAG, "_pthread_key_delete(%d)", key);
...
int _pthread_setspecific(pthread_key_t key,
                         const void *value)
{
  log_v(TAG, "_pthread_setspecific(%d, %d)", key, value);
  return pthread_setspecific(key, value);
}

void *_pthread_getspecific(pthread_key_t key)
{
  log_v(TAG, "_pthread_getspecific(%d)", key);
...
int _pthread_join(pthread_t thread,
                  void **value_ptr)
{
  log_v(TAG, "_pthread_join(%d, %d)", thread, value_ptr);
  return pthread_join(thread, value_ptr);
}

pthread_t _pthread_self(void)
{
  log_v(TAG, "_pthread_self()");
  return pthread_self();
}
...
{
  log_v(TAG, "_pthread_once(%d, %d)", once_control, init_routine);
  return pthread_once(once_control, init_routine);
}

int _pthread_cond_destroy(pthread_cond_t *cond)
{
  log_v(TAG, "_pthread_cond_destroy(%d)", cond);
...
{
  log_v(TAG, "_pthread_cond_wait(%d, %d)", cond, _MUTEX(mutex));
